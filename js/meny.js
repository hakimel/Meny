...
var Meny = {
        ...
        return (function(){
            ...
            function setupWrapper() {
                ...
                ['left', 'right', 'top', 'bottom'].forEach(function(pos) {
                    Meny.removeClass(dom.wrapper, 'meny-' + pos);
                });
            ...
            function setupMenu(starting) {
                ...
                if( supports3DTransforms ) {
                    ...
                    style[ Meny.prefix( 'transition' ) ] = Meny.lcPrefix('transform') + ' ' + config.transitionDuration +' '+ config.transitionEasing;
                }
                else {
            ...
            /**
             * Close any other open menys
             */
            function closeOtherMenys() {
                Meny._menys.forEach(function(meny) {
                    meny.close(dom.menu);
                });
            }
    ...
    /**
     * Prefixes a style property with the correct vendor (lowercase, '-' delimited)
     */
    lcPrefix: function(property, el) {
        var propertyUC = property.slice( 0, 1 ).toUpperCase() + property.slice( 1 ),
            vendors = [ 'Webkit', 'Moz', 'O', 'ms' ];
        ...
        for( var i = 0, len = vendors.length; i < len; i++ ) {
            ...
            if( typeof ( el || document.body ).style[ vendor + propertyUC ] !== 'undefined' ) {
                return ('-' + vendor + '-' + property.match(/([A-Z]?[^A-Z]*)/g).slice(0,-1).join('-')).toLowerCase();
            }
        }
